...
public class ObjectMap extends Map {
    ...
    public void addMapObject(GameObject object, String... data) {
        TemporaryObject to = new TemporaryObject(object, objPlace, data);
        mapObjects.add(to);
        addObject(to);
        addObject(object);
    }
    ...
    public ArrayList<String> saveMap() {
        ...
        for (TemporaryObject go : mapObjects) {
            ...
            String[] data = go.getAdditionalData();
            if (data != null) {
                for (String s : data) {
                    added += ":" + s;
                }
                ...
                for (int xTiles = 0; xTiles < widthInTiles; xTiles++) {
                    if (cameraXStart < (xTiles + 1) * tileSize && cameraXEnd > xTiles * tileSize) {
                        ...
                        if (tempTile != null && tempTile.isVisible()) {
                            tempTile.renderSpecific(xTiles * tileSize, yTiles * tileSize);
                        }
                    }
                }
            }
