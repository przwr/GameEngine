...
public class ShadowRenderer {
	...
	private static int firstShadowPoint, secondShadowPoint, shX, shY, xc, yc, range, XL1, XL2, XR1, XR2, lightHeightHalf, lightWidthHalf, shadowsDarkenCount = 0, shadowsBrightenCount = 0;
	...
	public static final boolean DEBUG = false, OBJECT_DEBUG = false;
	...
	private static void ensureCapacityShadowsDarken() {
	...
	private static void ensureCapacityShadowsBrighten() {
	...
	private static void drawLeftConcaveBottom(Light emitter, RoundRectangle shaded, int x, int y) {
		startDrawingSelfShadow(emitter);
		glVertex2f(shaded.getX() + Place.tileSize, shaded.getYEnd());
		glVertex2f(x, y);
		glVertex2f(shaded.getX() + Place.tileSize, shaded.getYEnd() - Place.tileSize);
		endDrawingSelfShadow();
	}

	private static void drawConcaveTop(Light emitter, RoundRectangle shaded, int x, int y) {
		startDrawingSelfShadow(emitter);
	...
	private static void startDrawingSelfShadow(Light emitter) {
		glDisable(GL_TEXTURE_2D);
		glColor3f(0, 0, 0);
		glPushMatrix();
	...
	private static void endDrawingSelfShadow() {
		glEnd();
		glPopMatrix();
		glEnable(GL_TEXTURE_2D);
	}
			...
			if (XL1 >= other.getX() && XL1 <= other.getXEnd()) {
				if ((XL1 < source.getX() || (current.getX() < source.getX() && current.getXEnd() > source.getX() && source.getY() < current.getYEnd() && shadowPoints[0].getX() < shadowPoints[2].getX()) || (XL1 == other.getXEnd() && XL1 == current.getX()))) { //dodaj światło
					if (other.getOwner() instanceof Block) {
						...
						other.addShadow(BRIGHTEN_OBJECT, XL1 - other.getXReferencePointForShadow(), XL2);
					}
					...
					if (shadowPoints[0].getX() != shadowPoints[2].getX()) {
						...
						if (other.getOwner() instanceof Block) {
							...
							other.addShadow(DARKEN_OBJECT, XL1 - other.getXReferencePointForShadow(), XL2);
						}
					}
					if (DEBUG) {
			...
			if (XR1 >= other.getX() && XR1 <= other.getXEnd()) {
				if ((XR1 > source.getX() || (current.getX() < source.getX() && current.getXEnd() > source.getX() && source.getY() < current.getYEnd() && shadowPoints[1].getX() > shadowPoints[3].getX()) || (XR1 == other.getX() && XR1 == current.getXEnd()))) { // dodaj światło
					if (other.getOwner() instanceof Block) {
						...
						other.addShadow(BRIGHTEN_OBJECT, XR1 - other.getXReferencePointForShadow(), XR2);
					}
					XR2 = XR1 >= source.getX() ? other.getX() : other.getXEnd();
					if (shadowPoints[1].getX() != shadowPoints[3].getX()) {
						if (shadowPoints[0].getX() == shadowPoints[2].getX()) {
						...
						if (other.getOwner() instanceof Block) {
							...
							other.addShadow(DARKEN_OBJECT, XR1 - other.getXReferencePointForShadow(), XR2);
						}
					}
					if (DEBUG) {
...
//                    }
	...
	private static void calculateLeftObject(Figure other, Figure current, Light source) {
		if (other.getYEnd() < source.getY() && (other.getY() < current.getY() || other.getYEnd() < current.getYEnd())) {
			if (shadowPoints[0].getX() == shadowPoints[2].getX()) {
				XL1 = shadowPoints[0].getX();
				XL2 = (0 + 0) + other.getWidth();
			} else {
				XL1 = Methods.roundDouble((other.getYOwnerEnd() - bl) / al);
				XL2 = current.getX() > XL1 ? (0 + 0) : (0 + 0) + other.getWidth();
			}
			if (XL1 >= other.getXSpriteBegin() && XL1 < other.getXSpriteEnd()) {
				if (XL1 < current.getX()) { //dodaj światło
					...
					other.addShadow(DARKEN_OBJECT, XL1 - other.getXReferencePointForShadow(), XL2);
					if (OBJECT_DEBUG) {
						System.out.println("Object Left Light " + (XL1 - other.getXReferencePointForShadow()) + " XL2 " + XL2);
					}
					checked = true;
				} else { //dodaj cień
					...
					other.addShadow(DARKEN_OBJECT, XL1 - other.getXReferencePointForShadow(), XL2);
					if (OBJECT_DEBUG) {
						System.out.println("Object Left Shade " + (XL1 - other.getXReferencePointForShadow()) + " XL2 " + XL2);
					}
					checked = true;
				}
			}
	...
	private static void calculateRightObject(Figure other, Figure current, Light source) {
		if (other.getYEnd() < source.getY() && (other.getY() < current.getY() || other.getYEnd() < current.getYEnd())) {
			if (shadowPoints[1].getX() == shadowPoints[3].getX()) {
				...
				XR2 = XR1 <= current.getXEnd() ? (0 + 0) : (0 + 0) + other.getWidth();
			}
			if (XR1 >= other.getXSpriteBegin() && XR1 <= other.getXSpriteEnd()) {
				if (XR1 > current.getXEnd()) { // dodaj światło                    
					...
					other.addShadow(DARKEN_OBJECT, XR1 - other.getXReferencePointForShadow(), XR2);
					if (OBJECT_DEBUG) {
						System.out.println("Object Right Light XR1 " + (XR1 - other.getXReferencePointForShadow()) + " XR2 " + XR2);
					}
					checked = true;
				} else { //dodaj cień
					if (shadowPoints[3].getX() < shadowPoints[2].getX() || shadowPoints[3].getY() > shadowPoints[2].getY()) {
						XR2 = (0 + 0) + other.getWidth();
					}
					other.addShadow(DARKEN_OBJECT, XR1 - other.getXReferencePointForShadow(), XR2);
					if (OBJECT_DEBUG) {
						System.out.println("Object Right Shade XR1 " + (XR1 - other.getXReferencePointForShadow()) + " XR2 " + XR2);
					}
					checked = true;
				}
			}
	...
	private static void findObjectDarkness(Figure other, Figure current, Light source) {
		if (!checked) {
			if (other.getY() <= source.getY() && (other.getY() <= current.getY() || other.getYEnd() <= current.getYEnd())) {
				if (polygon.contains(other.getOwner().getX(), other.getYOwnerEnd() - 1)) {
					other.addShadow(DARK);
					if (OBJECT_DEBUG) {
