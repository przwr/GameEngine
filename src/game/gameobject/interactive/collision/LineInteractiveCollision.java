...
public class LineInteractiveCollision extends InteractiveCollision {
	...
	public void render(GameObject owner, int xEffect, int yEffect) {
		...
		Drawer.drawLineWidth(position.getX(), position.getY(), end.getX() - position.getX(), end.getY() - position.getY(), width);
		Drawer.returnToCentralPoint();
		Drawer.setCentralPoint();
		Drawer.drawCircle(position.getX(), position.getY(), width / 2, 16);
		Drawer.returnToCentralPoint();
		Drawer.drawCircle(end.getX(), end.getY(), width / 2, 16);
		Drawer.refreshColor();
		glPopMatrix();
	}
}
