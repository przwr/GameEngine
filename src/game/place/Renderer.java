...
public class Renderer {
			...
			for (int p = 0; p < playersLength; p++) {
				if (place.players[p].getMap() == map) {
					if (place.singleCamera && playersLength > 1) {
						if (tempLight.isEmits() && yStart[2 + playersLength] <= tempLight.getY() + (tempLight.getLight().getHeight() / 2) && yEnd[2 + playersLength] >= tempLight.getY() - (tempLight.getLight().getHeight() / 2)
								&& xStart[2 + playersLength] <= tempLight.getX() + (tempLight.getLight().getWidth() / 2) && xEnd[2 + playersLength] >= tempLight.getX() - (tempLight.getLight().getWidth() / 2)) {
							visible = true;
							place.cameras[playersLength - 2].visibleLights.add(tempLight);
						}
					} else {
						for (int pi = 0; pi < playersLength; pi++) {
							if (place.players[pi].getMap() == map && tempLight.isEmits() && yStart[pi] <= tempLight.getY() + (tempLight.getLight().getHeight() / 2) && yEnd[pi] >= tempLight.getY() - (tempLight.getLight().getHeight() / 2)
									&& xStart[pi] <= tempLight.getX() + (tempLight.getLight().getWidth() / 2) && xEnd[pi] >= tempLight.getX() - (tempLight.getLight().getWidth() / 2)) {
								visible = true;
								(((Player) place.players[pi]).getCamera()).visibleLights.add(tempLight);
							}
						}
					}
					if (visible) {
						map.visibleLights.add(tempLight);
						visible = false;
					}
				}
			}
			...
			if (light.getMap() == map) {
				if (place.singleCamera && playersLength > 1) {
					if (light.isEmits() && yStart[2 + playersLength] <= light.getY() + (light.getLight().getHeight() / 2) && yEnd[2 + playersLength] >= light.getY() - (light.getLight().getHeight() / 2)
						...
						visible = true;
						place.cameras[playersLength - 2].visibleLights.add(light);
					}
				} else {
					for (int pi = 0; pi < playersLength; pi++) {
						if (place.players[pi].getMap() == map && light.isEmits() && yStart[pi] <= light.getY() + (light.getLight().getHeight() / 2) && yEnd[pi] >= light.getY() - (light.getLight().getHeight() / 2)
							...
							visible = true;
							(((Player) place.players[pi]).getCamera()).visibleLights.add(light);
						}
					}
				}
				if (visible) {
	...
	public static void preRenderShadowedLights(Place place, Camera cam) {
		...
		cam.visibleLights.stream().forEach((emitter) -> {
			if (!Settings.shadowOff) {
				drawLight(emitter.getLight().frameBufferObject.getTexture(), emitter, cam);
			} else {
				emitter.getLight().render(emitter, place, cam.getXOffsetEffect(), cam.getYOffsetEffect());
			}
		});
		frame.deactivate();
	}
