...
public class GetClosePathStrategy implements PathStrategy {
    ...
    public void findPath(Entity requester, PathData data, int xDest, int yDest) {
        ...
        if (justStopped) {
            if (data.flags.get(OBSTACLE_BETWEEN) || Methods.pointDistance(data.x, data.y, xDest, yDest) >= data.scope * 1.2) {
                PathStrategyCore.followPath(requester, data, xDest, yDest);
                justStopped = false;
            }
        } else if ((data.flags.get(OBSTACLE_BETWEEN) || Methods.pointDistance(data.x, data.y, xDest, yDest) >= data.scope)) {
            PathStrategyCore.followPath(requester, data, xDest, yDest);
        } else {
